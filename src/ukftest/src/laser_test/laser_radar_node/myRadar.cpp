...
void OpenRadar::VFH()
{
	...
	*/
	...
	if (H[s_idx] - max_H < equal_block_thres)
	{
		...
		else if (min_H > block_thres || sumH > 7100 ) 
		{
		...
		{
			//v_scale = (1 - H[s_idx]/H[b_idx]) * 100;
			v_scale = (H[b_idx] - H[s_idx]) / 23;
			v_scale = v_scale>1? 1 : v_scale;
			v_scale = v_scale<0? 0 : v_scale;
			v_angle = (-120+b_idx*polarH_resolution)*M_PI/180.0f + M_PI;
		}
		...
		else 
		{
			...
			if(isblock)
			{
				...
				lasermsg.avoid.x = v_scale*sin(v_angle + 0.5*M_PI); 
				lasermsg.avoid.y = v_scale*cos(v_angle + 0.5*M_PI); 
				lasermsg.dt = dt;
