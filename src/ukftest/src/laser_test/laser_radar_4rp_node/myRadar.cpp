...
OpenRadar::OpenRadar(void)
{
	...
	pub=n2.advertise<ukftest::laserInfo>("uav_laser",10);
	pub_xy = n2.advertise<ukftest::laserPoint>("laser_point",10);
...
void OpenRadar::VFH()
{
	...
	{
		if (i >	(sector+1)*polarH_resolution-1)
			sector++;
	...
	for(int i = 0 ; i < RadarDataCnt ; i ++)
	{
		cout<<"RadarRho"<< i << ":" <<RadarRho[i] <<endl;	
	}
	for(int i = 0 ; i < polarH_length ; i ++)
	{
		cout<<"H"<< i << ":" <<H[i] <<endl;	
	...
	*/
	...
	if (H[s_idx] - max_H < equal_block_thres)
	{
		...
		if(H[b_idx] - H[s_idx] == 0)
		{
			isblock = false;
		}
		else
		{
			v_scale = (H[b_idx] - H[s_idx]) / 40;
			v_scale = v_scale>1? 1 : v_scale;
			v_scale = v_scale<0? 0 : v_scale;
			v_angle = 2*M_PI - (b_idx*polarH_resolution)*M_PI/180.0f;
			if( v_angle > M_PI) v_angle -= M_PI;
			else v_angle += M_PI;
		}
		if(timeInit)
		{
		...
		else 
		{
			...
			time_old = ros::Time::now();
			// msg
			ukftest::laserInfo lasermsg;
			lasermsg.header.stamp = ros::Time::now();
			lasermsg.header.frame_id = "laser_avoid";

			if(isblock)
			{
			

				lasermsg.avoid.x = v_scale*cos(v_angle); 
				lasermsg.avoid.y = v_scale*sin(v_angle); 
				lasermsg.dt = dt;
				lasermsg.isBlocking = 1;
			...
			{
				lasermsg.avoid.x = 0.0f;
				lasermsg.avoid.y = 0.0f;
				lasermsg.dt = dt;
				lasermsg.isBlocking = 0;
				pub.publish(lasermsg);
			}
			//debug
			cout<< "polarH_length:"<<polarH_length<<endl;
...
void OpenRadar::laser_callback(const sensor_msgs::LaserScan::ConstPtr& msg)
{
	...
	if (receive_state == 1)
	{
		...
		range_min = my_range_min; //20cm
		range_max = my_range_max; //max 60cm

		cout<< "my_range_min " << range_min<<endl;
		cout<< "my_range_max " << range_max<<endl;
...
void OpenRadar::DrawRadarData()
{
	...
	pt2.x = DisplayDx+line_length*v_scale*cos(v_angle); 
	pt2.y = DisplayDy+line_length*v_scale*sin(v_angle);
