...
namespace estimation
{
		...
		ukf_sent_counter_(0)
	{
		...
		nh_private.param("freq", freq, 25.0); 
		...
		if (vo_used_){
			...
			vo_sub_ = nh.subscribe("board_pose", 10, &OdomEstimationNode::voCallback, this);
		}
	...
	void OdomEstimationNode::voCallback(const VoConstPtr& vo)
	{
		...
		*/
		...
		// q_ct
		vo_data(0) = vo -> pose.orientation.w;
		vo_data(1) = vo -> pose.orientation.x;
		vo_data(2) = vo -> pose.orientation.y;
		vo_data(3) = vo -> pose.orientation.z;

		// T_ct
		vo_data(4) = vo -> pose.position.x * 100;  
		vo_data(5) = vo -> pose.position.y * 100; 
		vo_data(6) = vo -> pose.position.z * 100; 

		//std::cout<<"NOW "<<std::endl;
		//for (int i = 0 ; i < vo_data.size() ; i++) std::cout<< vo_data(i)<<std::endl;
