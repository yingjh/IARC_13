...
* All rights reserved.
*
* File:	uav_b_laser_controller.cpp
...
// avoid ctrl
PID *ctrl_avoid_x, *ctrl_avoid_y, *ctrl_avoid_z;
...
vector3f acc_e; // final control variables
...
int main (int argc, char** argv)
{
	...
	*/
	...
	//ctrl_avoid_z = new PID(Kp_avoid_vel, Ki_avoid_vel, Kd_avoid_vel, -30, 30, -800, 800); 

	ctrlpub = nh.advertise<geometry_msgs::Quaternion>("/board_ctrl",10);
	ukfsub = nh.subscribe("laser_ukf", 10, ukfCallback);
...
void ukfCallback(const ukftest::ukfData& ukf)
{
	...
	else
	{
		...
		v_b[0] = ukf.avoid.x *100; //cm/s
		v_b[1] = ukf.avoid.y *100;
		v_b[2] = 0;
		...
		acc_b[1] = ctrl_avoid_y -> update(imuVY, delta_t, &error_speed_x, &p_speed_x, &i_speed_x, &d_speed_x); //control
		acc_b[2] = 0;
