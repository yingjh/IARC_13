...
* All rights reserved.
*
* File:	uav_track_neg_controller.cpp
* Brief: 
* Version: 1.0
* Author: YING Jiahang
...
const float targetPosition[3] = {0.0,0.0,15.0};
...
int main (int argc, char** argv)
{
	...
	ros::init(argc, argv, "uav_track_neg_controller_node");
	ros::NodeHandle nh;
	...
	nh.param("pid_gain_track", pid_gain, 1.0);

	nh.param("controlLimit_track", controlLimit, 5.0);
	...
	*/
	...
	ctrlpub = nh.advertise<ukftest::trackCtrl>("track_ctrl",10);
	ukfsub = nh.subscribe("/neg_estimation_node/odom_combined", 10, ukfCallback);
	debug_pub = nh.advertise<irobot_tracker::trackerDebug>("tracker_debug",10);
	
	cout<< "uav track controller start!"<<endl;
	ros::spin();
	cout<< "uav track controller shutdown!"<<endl;
	return 1;
}    
...
void ukfCallback(const ukftest::ukfData& ukf)
{
	...
	ukftest::trackCtrl outMsg;
	outMsg.header.stamp = ros::Time::now();
	outMsg.header.frame_id = "cam_track_ctrl";
	if(ukf.isctrl ==1)
	{
	...
	if(is_debug_on)
	{
		...
		cout<< "isTracking:" << outMsg.isTracking << endl;
