...
// Ros
ros::Subscriber avoidsub, tracksub, randomsub, landsub ,boundarysub, takeoffsub, statussub;
...
void spinCallback(const ros::TimerEvent& e)
{
	...
	allTimeCnt++;
	cout <<"pitchCnt: "<< pitchCnt << endl;
	if(uav_status == 1 && pitchCnt > (2*30) ) 
	{
		requestMsg.data = 2.0;
		requestpub.publish(requestMsg);
		takeoffCnt = 0;
		cout<< "state: request takeoff!" <<endl;
		statusMsg.data = 9.0;
	...
	else if(pitchCnt>(2*30))
	{
		outMsg.z = (int)0;         //yaw_rate  
		outMsg.x = (int)0;         //pitch
		outMsg.y = (int)0;         //roll
		outMsg.w = (int)10;         //vel
		cout<< "state: need landing!"<<endl;
		statusMsg.data = 8.0;
		statuspub.publish(statusMsg);
	}
	else if(isTrack && takeoffCnt > 1)
	{
		trackCnt++;
		if(track_x > 200) pitchCnt++;
