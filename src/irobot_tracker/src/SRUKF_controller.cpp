...
matrix3f R_bc, R_bb_f, R_bb_f_t; // suppose to be constant, will be initialized in main function
...
int main (int argc, char** argv)
{
    ...
    matrix3f_set_value(R_bc, 0, -1, 0,
                             1,  0, 0,
                             0,  0, 1);
    matrix3f_set_value(R_bb_f, -1,  0,  0,
                                0,  1,  0,
                                0,  0, -1);
    get_matrix3f_transpose(R_bb_f_t, R_bb_f);
              ...
              const irobot_tracker::trackInfo::ConstPtr& msg2)
{
    ...
    a_e[0] = msg1 -> linear_a.x * 100;   
    a_e[1] = msg1 -> linear_a.y * 100;  
    a_e[2] = msg1 -> linear_a.z * 100;  
    
    w_b[0] = msg1 -> angular_v.x/180.0f*M_PI; 
    ...
    if (msg2 -> isTracking == true)
    {
        ...
        T_tc[0] = msg2 -> pose.position.x * 100 ;  
        T_tc[1] = msg2 -> pose.position.y * 100 ; 
    ...
    if (plotCount %5==0)
		cout << "a_t_x " << a_t[0]
			 << "| a_t_y" << a_t[1]
			 << "| a_t_z" << a_t[2] << endl;
        float_limit(T_tc_new[0], 300);
        float_limit(T_tc_new[1], 300);
        float_limit(T_tc_new[2], 400);
        s(0) = T_tc_new[0]; s(1) = T_tc_new[1]; s(2) = T_tc_new[2]; 
	...
	*/
    ...
    target_velX = ctrl_x -> update(ukf->state_post(0), delta_t, &d_error, &d_pval, &d_ival, &d_dval); //control roll
    ...
    target_velY = ctrl_y -> update(ukf->state_post(1), delta_t, &d_error, &d_pval, &d_ival, &d_dval);   //control pitch
    target_velZ = ctrl_z -> update(ukf->state_post(2), delta_t, &d_error, &d_pval, &d_ival, &d_dval);   //control vel

    ctrl_dx -> set_point(target_velX);
    ctrl_dy -> set_point(target_velY);
    ctrl_dz -> set_point(target_velZ);

    target_accX = ctrl_dx -> update(ukf->state_post(7), delta_t, &d_error, &d_pval, &d_ival, &d_dval); //control roll
    ...
    target_accY = ctrl_dy -> update(ukf->state_post(8), delta_t, &d_error, &d_pval, &d_ival, &d_dval);   //control pitch
    target_accZ = ctrl_dz -> update(ukf->state_post(9), delta_t, &d_error, &d_pval, &d_ival, &d_dval);   //control vel

    ctrl_ddx -> set_point(target_accX);
    ctrl_ddy -> set_point(target_accY);
    ctrl_ddz -> set_point(target_accZ);
    target_angleX = ctrl_ddx -> update(a_t[0], delta_t, &d_error, &d_pval, &d_ival, &d_dval);   //control vel
    ...
    target_angleY = ctrl_ddy -> update(a_t[1], delta_t, &d_error, &d_pval, &d_ival, &d_dval);   //control vel
    target_angleZ = ctrl_ddz -> update(a_t[2], delta_t, &d_error, &d_pval, &d_ival, &d_dval);   //control vel
    ...
    if (plotCount %1==0)
    {
        ...
        debug_msg.imu_ort.w = yaw;
        debug_msg.imu_ort.x = pitch;
        debug_msg.imu_ort.y = roll;
        ...
        matrix3f_multi_matrix3f(R_fc, R_bb_f, R_tc); // R_fc = R_bb_f * R_tc
        DCM_to_quat(q_tc_filter, R_fc);		// use q_tc_filter as tmp storage
